...
def split(configfile,inputfile,format,numjobs,cluster):
	if not os.path.exists(inputfile+".dir"):
		...
		for i in range (0,numjobs):
			outputfile[i]={}
			outputfile[i]['ofile']=inputfile+".dir/"+os.path.basename(inputfile)+"."+str(i)+".fa"
...
def join(configfile,inputfile,clean):
...
def plot(configfile,inputfile):
	...
	for g in genomes:
		...
		ax.legend()
		plt.savefig(outputdir+"/"+os.path.basename(inputfile)+'.seqlength.'+g+'.png')   # save the figure to file
		plt.close()		
	...
	plt.savefig(outputdir+"/"+os.path.basename(inputfile)+'.frameshift.png')   # save the figure to file
	plt.close()		
	...
	for g in genomes:
		...
		if os.path.isfile(fname):
			...
			plt.savefig(outputdir+"/"+os.path.basename(inputfile)+'.deconcatrounds.'+g+'.png')   # save the figure to file
			plt.close()		
			#score ###########################################################################
			fig, ax = plt.subplots()
			...
			plt.savefig(outputdir+"/"+os.path.basename(inputfile)+'.deconcatscore.'+g+'.png')   # save the figure to file
			plt.close()		
			#cases ###########################################################################
			fig, ax = plt.subplots()
